...
// TEMP VARIABLES TO DO SERVO SIM
...
float spin_time = 0, backup_spin_time = 0;
...
const float servo_resolution = 0.003970624048 * 2;  // rad
int multiple = 0;                                   //
...
float M_Net;            //
float Prev_n = 0;       // Omega / (2*pi)
float n = 0;            // Omega / (2*pi)
float Omega = 0;        // Moment_A / I_Total
float Prev_Omega = 0;   // Moment_A / I_Total
float Alpha_Omega = 0;  //

// Define control variables
float Kp = 5.9e-6, Ki = 20e-7, Kd = 0;  // Proportional, Integral ,Derivative  Gains
//float Kp = 3e-5, Ki = 0, Kd = 0;  // Proportional, Integral ,Derivative  Gains
float setpoint = 0;  // Goal value (rpm)
...
void setup() {
  ...
  myFile = SD.open("ARC.txt", FILE_READ);
}
...
void loop() {
  if (myFile) {
    ...
    if (Cur_Time >= 3 && (reached_target_1 == false || reached_target_2 == false)) {
      // Find the error between the setpoint and the current RPM
      error = setpoint - n;
      derivative = (error - prev_error) / dt;
      integral = integral + error * dt;
      AoA += Kp * error + Ki * integral + Kd * derivative;
      ...
      if ((abs(n) > (abs(setpoint) - 2)) && (abs(n) < (abs(setpoint) + 2))) {
        spin_time += dt;
      }
      ...
      if (spin_time >= hold_time || backup_spin_time >= backup_hold_time) {
        setpoint = -setpoint;
    ...
    Serial.print(spin_time, 4);
    Serial.print(" | ");
    Serial.print("Backup Spin Time = ");
    Serial.print(backup_spin_time, 4);
    Serial.print(" | ");
    Serial.print("Time = ");
